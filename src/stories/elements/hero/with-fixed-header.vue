<template>
  <div class="dp-theme-dark">
    <dp-navbar style="position: absolute; z-index: 1; width: 100%; padding: 0 35px; left: 0; top: auto;"
               :brand="brand"
               :items="items">
      <template slot="right">
        <dp-button
          :href="'/sg/review'"
          class="dp-button--ghost-primary dp-button--sm">
          <span>Review</span>
          <i class="dp-button__icon--right fa fa-pencil dp-hidden__mobile"></i>
        </dp-button>
        <dp-button
          :href="'/sg/login'"
          class="dp-button--primary dp-button--sm">
          <span>Log In</span>
        </dp-button>
      </template>
    </dp-navbar>
    <main>
      <dp-hero
        ...
        <template slot="content">
          <div :style="isDesktop ? 'margin-top: 76px;' : 'margin-top: 60px;'"
               v-html="content" />
        </template>
      </dp-hero>
    </main>
...
const navbarItems = [
  ...
  {
    'label': 'Promotions',
    'url': 'https://www.xxxxxx.com/sg/promotions',
    'active': false,
    'megamenu': []
  },
  {
...
export default {
  data () {
    return {
      isCurved: boolean('isCurved', true),
      curveColor: text('curveColor', '#FFFFFF'),
      size: select('size', sizeOptions, ''),
      imageUrl: text('imageUrl', 'https://picsum.photos/1024/480?random'),
      imageAlt: text('imageAlt', 'My Hero Image'),
      overlayOpacity: select('overlayOpacity', opacityOptions, '0'),
      items: array('Items', navbarItems),
      content: text('Content Slot', '<div class="container-fluid"><div class="row"><div class="col-xs-6 col-xs-offset-3"><h1 class="dp-title dp-title--light" style="text-align: center;">Hello, World!</h1></div></div></div>'),
      brand: brand
    }
  },
  computed: {
